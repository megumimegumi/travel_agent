...
class PlanningAgent(BaseAgent):
    ...
    def _build_system_prompt(self, request: TravelRequest, weather_info: str, route_info: str, feedback: str = "", user_history: str = "") -> str:
        ...
        if user_history:
            history_section = f"""
            📚 【用户AI画像与历史概览】
            以下是关于该用户的评测与偏好概述（仅供参考，用于推断用户潜在偏好）：
            {user_history}
            
            🪄 **重要个性化指令**：
            请分析上述关于该用户的**消费水平**、**行程天数节奏**以及**偏好标签**。
            ...
            2. 无论本次预算如何变动，你**绝对必须**在返回的 JSON 的 `special_tips` 字段里，必须且只能加入一个名为 `"专属定制说明"` 的数组字段并保证其位于最前面 ！
               该数组必须包含以下 2 个严格格式的字符串元素（禁止用单纯的天气、节奏内容来随意替换它，必须是明确围绕用户的“画像与评价”展开的内容）：
               - "【用户概述】：根据您近期的游玩偏好和长期画像，总结发现您在日常旅行中倾向于 [描述用户主要消费观念与旅行风格]。"
        ...
        important_rule_history = ""
        if user_history:
            important_rule_history = "- 无论预算何种情况，必须毫无条件地生成【用户概述】、【深度定制】这特定的2句话放在 special_tips 的 `\"专属定制说明\"` 数组内！绝对不要私自将其替换位天气或预算警告。若有其它预算警告需另开如 `\"预算警告\"` 或 `\"天气建议\"` 分类。"
        
        example_special_tips = """
            "special_tips": {
                "交通": ["建议提前购买高铁票"],
                "防坑指南": ["不要轻信路边一日游销售"]
            }
        ...
        if user_history:
            ...
            "special_tips": {
                "专属定制说明": [
                    ...
                    "【用户概述】发现您在日常旅行中倾向于高端的奢华住宿与轻松闲适的游览节奏。",
